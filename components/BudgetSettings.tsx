    ...
    <div className="space-y-6 md:space-y-8 animate-slide-up max-w-4xl mx-auto pb-32 px-1 sm:px-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        <div className="group relative p-6 md:p-8 bg-white dark:bg-neutral-900 border-2 border-primary/40 rounded-[2rem] md:rounded-[2.5rem] space-y-6 md:space-y-8 shadow-xl transition-all hover:border-primary">
          ...
          <div className="flex flex-col items-center pt-2">
            <div className="relative">
              ...
              <button onClick={() => fileInputRefA.current?.click()} className="absolute -bottom-1 -right-1 md:-bottom-2 md:-right-2 p-3 bg-primary text-neutral-950 rounded-xl md:rounded-2xl shadow-glow hover:scale-110 transition-transform active:scale-90">
                <Camera className="w-4 h-4 md:w-5 md:h-5" strokeWidth={2.5} />
              </button>
          ...
          <div className="space-y-4 md:space-y-5">
            <div className="space-y-1.5 md:space-y-2">
              ...
              <input type="text" value={nameA} onChange={e => setNameA(e.target.value)} className="w-full bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-xl md:rounded-2xl px-5 py-3 md:px-6 md:py-4 text-base md:text-lg font-bold text-neutral-900 dark:text-white outline-none focus:border-primary transition-all" />
            </div>
            <div className="space-y-1.5 md:space-y-2">
              ...
              <div className="relative">
                ...
                <input type="number" value={incomeA} onChange={e => setIncomeA(e.target.value)} className="w-full pl-12 md:pl-14 pr-5 py-3 md:py-5 bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-xl md:rounded-2xl text-xl md:text-2xl font-display font-black text-neutral-900 dark:text-white outline-none focus:border-primary transition-all" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="group relative p-6 md:p-8 bg-white dark:bg-neutral-900 border-2 border-blue-500/40 rounded-[2rem] md:rounded-[2.5rem] space-y-6 md:space-y-8 shadow-xl transition-all hover:border-blue-500">
          ...
          <div className="space-y-4 md:space-y-5">
            <div className="space-y-1.5 md:space-y-2">
              <label className="text-[8px] md:text-[10px] font-black text-neutral-400 uppercase tracking-widest ml-1">Nome de Exibição</label>
              <input type="text" value={nameB} onChange={e => setNameB(e.target.value)} className="w-full bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-xl md:rounded-2xl px-5 py-3 md:px-6 md:py-4 text-base md:text-lg font-bold text-neutral-900 dark:text-white outline-none focus:border-blue-500 transition-all" />
            </div>
            <div className="space-y-1.5 md:space-y-2">
              <label className="text-[8px] md:text-[10px] font-black text-neutral-400 uppercase tracking-widest ml-1">Renda Mensal</label>
              <div className="relative">
                <span className="absolute left-5 top-1/2 -translate-y-1/2 text-neutral-400 font-bold text-sm">R$</span>
                <input type="number" value={incomeB} onChange={e => setIncomeB(e.target.value)} className="w-full pl-12 md:pl-14 pr-5 py-3 md:py-5 bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-xl md:rounded-2xl text-xl md:text-2xl font-display font-black text-neutral-900 dark:text-white outline-none focus:border-blue-500 transition-all" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="p-6 md:p-8 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-[2rem] md:rounded-[2.5rem] shadow-sm">
        <div className="flex items-center space-x-4 mb-6 md:mb-8">
          <div className="w-10 h-10 md:w-12 md:h-12 bg-primary/10 rounded-xl md:rounded-2xl flex items-center justify-center text-primary border border-primary/20 shrink-0"><Sun className="w-5 h-5 md:w-6 md:h-6" /></div>
      ...
      <div className="sticky bottom-24 md:static bg-neutral-950 border border-neutral-800 p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] flex flex-col md:flex-row items-center justify-between gap-6 shadow-[0_0_60px_rgba(0,0,0,0.6)] z-50">
        ...
        <button onClick={handleSaveAll} disabled={isSyncing || localSaving} className={`w-full md:w-auto min-w-[200px] md:min-w-[280px] flex items-center justify-center space-x-2 md:space-x-3 px-8 py-5 md:px-12 md:py-7 rounded-[1.5rem] md:rounded-[2.5rem] font-black uppercase text-[10px] md:text-xs tracking-[0.2em] md:tracking-[0.3em] transition-all active:scale-95 ${isSyncing || localSaving ? 'bg-emerald-500 text-white animate-pulse cursor-wait' : 'bg-primary text-neutral-950 shadow-glow hover:bg-yellow-300'}`}>
          {isSyncing || localSaving ? <><Loader2 className="animate-spin" size={18} /><span>Sincronizando...</span></> : <span>Salvar na Nuvem</span>}
        </button>
      </div>
